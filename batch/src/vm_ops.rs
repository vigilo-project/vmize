...
///
/// This trait enables dependency injection for testing batch logic
...
pub mod mock {
    ...
    impl MockVmOps {
        ...
        pub fn with_ssh_ok(self, output: &str) -> Self {
            self.ssh_results.lock().unwrap().push_back(Ok(output.to_string()));
            self
        }

        /// Add a failing ssh() response
        pub fn with_ssh_err(self, msg: &str) -> Self {
            self.ssh_results.lock().unwrap().push_back(Err(anyhow::anyhow!(msg.to_string())));
            self
        }
        ...
        pub fn with_stream_outputs(self, lines: Vec<&str>) -> Self {
            self.stream_outputs.lock().unwrap().push_back(
    ...
    impl VmOps for MockVmOps {
        async fn run(&self, _options: VmOptions) -> Result<VmRecord> {
            ...
            self.run_record.lock().unwrap().clone().ok_or_else(|| anyhow::anyhow!("no run record configured"))
        }

        async fn ssh(&self, vm_id: &str, command: &str) -> Result<String> {
            self.ssh_commands.lock().unwrap().push((vm_id.to_string(), command.to_string()));
        ...
        where
            F: FnMut(String) + Send,
        {
            self.stream_commands.lock().unwrap().push(command.to_string());
...
mod tests {
    ...
    fn mock_vm_ops_cp_to_records_calls() {
        ...
        mock.cp_to("vm0", "/local/path", "/remote/path", true).unwrap();

        let calls = mock.cp_to_calls.lock().unwrap();
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0], ("vm0".to_string(), "/local/path".to_string(), "/remote/path".to_string(), true));
    }
    ...
    fn mock_vm_ops_ssh_stream_outputs_lines() {
        let mock = MockVmOps::new()
            .with_stream_outputs(vec!["line1", "line2", "line3"]);

        let record = mock_vm_record("vm0");

        let mut collected = Vec::new();
        mock.ssh_stream(&record, "echo test", |line| collected.push(line)).unwrap();
