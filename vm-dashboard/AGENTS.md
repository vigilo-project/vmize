...
`vm-dashboard` is a single-binary Rust crate that provides a browser UI for `vm-batch`.
...
1. `vm-dashboard --port 8080` starts successfully.
2. Valid job path can be added; queue shows `name`/`description` from `job.json`.
3. Invalid path is rejected with a clear error and no queue mutation.
4. A 5th queued job is rejected (queue max is 4).
5. Queued jobs can be removed before run.
6. `Run All` starts queued jobs in parallel and prevents duplicate starts.
...
- `cargo build --release -p vm-dashboard`: build release binary.
- `cargo test --lib -p vm-dashboard`: unit tests only.
- `cargo test --test api -p vm-dashboard`: HTTP/API integration tests (no QEMU).
- `VM_DASHBOARD_IT=1 cargo test --test api run_api_run_job_succeeds -p vm-dashboard -- --nocapture`: VM-required end-to-end run path.
- `cargo fmt -p vm-dashboard && cargo clippy -p vm-dashboard --all-targets --all-features`: format and lint before PR.

## Coding Style & Concurrency Rules
Use Rust 2021 defaults (`snake_case`, `PascalCase`, `Result`-first error handling). Do not hold `RwLock` guards across `.await`. Preserve axum route syntax (`/api/jobs/{id}`) and SSE replay behavior (recent-event replay + live stream).
