//! HTTP API integration tests for vm-dashboard.
//!
//! These tests spawn the real `vm-dashboard` binary on a free port, exercise
//! every endpoint, and verify responses.  No QEMU is required — `POST /api/run`
//! is only tested end-to-end when `VM_DASHBOARD_IT=1` is set.
//!
//! Requires:
//!   - `vm-dashboard` binary to be built (debug or release)
...
fn dashboard_bin() -> String {
    std::env::var("CARGO_BIN_EXE_vm-dashboard").unwrap_or_else(|_| {
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../target/debug/vm-dashboard")
            .to_string_lossy()
            .into_owned()
    })
}
...
fn example_job_dir() -> String {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../vm-batch/example/job1")
        .canonicalize()
        .expect("vm-batch/example/job1 must exist")
        .to_string_lossy()
        .into_owned()
}
    ...
    fn start() -> Self {
        ...
        loop {
            if Instant::now() > deadline {
                panic!("vm-dashboard did not become ready within 5 s on port {port}");
            }
            if client
...
fn serve_dashboard_returns_html() {
    ...
    assert!(
        body.contains("vm-dashboard"),
        "title 'vm-dashboard' not found in HTML"
    );
    assert!(
...
/// Skipped unless `VM_DASHBOARD_IT=1` is set (requires QEMU).
#[test]
fn run_api_run_job_succeeds() {
    if std::env::var("VM_DASHBOARD_IT").is_err() {
        eprintln!("Skipping VM end-to-end test: set VM_DASHBOARD_IT=1 to run.");
        return;
    }
