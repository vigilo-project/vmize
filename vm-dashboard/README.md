...
`vm-dashboard` is a web-based job runner for [`vm-batch`](../vm-batch). It replaces the terminal-only `--split-live` mode with a browser UI that has no TTY requirement and works over SSH.

## Minimum Goal

The MVP is considered complete when all of the following pass end-to-end in a browser:

1. **Start server** — `vm-dashboard --port 8080` starts without error.
...
./target/release/vm-dashboard            # default port 8080
./target/release/vm-dashboard --port 9090
```
...
A job directory is the same format as `vm-batch`:
...
cargo test --lib -p vm-dashboard

# All tests including HTTP API (no VM needed — only tests the HTTP layer)
cargo test --test api -p vm-dashboard

# Full end-to-end (requires QEMU — actually runs a VM)
VM_DASHBOARD_IT=1 cargo test --test api run_api_run_job_succeeds -p vm-dashboard -- --nocapture
```
